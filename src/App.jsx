...
const store = {
  async load(email) {
    try {
      const raw = localStorage.getItem(`expenses:${email}`);
      return raw ? JSON.parse(raw) : [];
    } catch { return []; }
  },
  async save(email, expenses) {
    try { localStorage.setItem(`expenses:${email}`, JSON.stringify(expenses)); } catch {}
  },
};
...
const authStore = {
  ...
  async save(session) {
    try { localStorage.setItem("auth:session", JSON.stringify(session)); } catch {}
  },
  async clear() {
...
const usersStore = {
  async load() {
    try {
      const raw = localStorage.getItem("users:registry");
      return raw ? JSON.parse(raw) : {};
    } catch { return {}; }
  },
  async save(users) {
    try { localStorage.setItem("users:registry", JSON.stringify(users)); } catch {}
  },
};
